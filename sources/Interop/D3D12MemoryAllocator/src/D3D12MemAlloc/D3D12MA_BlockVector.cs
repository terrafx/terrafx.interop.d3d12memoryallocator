...
namespace TerraFX.Interop
{
    ...
    {
        readonly D3D12MA_AllocatorPimpl* m_hAllocator;
        ...
        D3D12MA_Vector<Pointer<D3D12MA_NormalBlock>> m_Blocks;

        [NativeTypeName("UINT")]
        uint m_NextBlockId;

        public D3D12MA_BlockVector(D3D12MA_AllocatorPimpl* hAllocator, D3D12_HEAP_TYPE heapType, D3D12_HEAP_FLAGS heapFlags, [NativeTypeName("UINT64")] ulong preferredBlockSize, [NativeTypeName("size_t")] nuint minBlockCount, [NativeTypeName("size_t")] nuint maxBlockCount, bool explicitBlockSize)
        {
        ...
        public bool IsEmpty()
        {
            using var @lock = new D3D12MA_MutexLockRead((D3D12MA_RW_MUTEX*)Unsafe.AsPointer(ref m_Mutex), m_hAllocator->UseMutex());
            return m_Blocks.empty();
        }

        [return: NativeTypeName("HRESULT")]
        public int Allocate([NativeTypeName("UINT64")] ulong size, [NativeTypeName("UINT64")] ulong alignment, D3D12MA_ALLOCATION_DESC* allocDesc, [NativeTypeName("size_t")] nuint allocationCount, D3D12MA_Allocation** pAllocations)
        {
            nuint allocIndex;
            HRESULT hr = S_OK;

            {
                using var @lock = new D3D12MA_MutexLockWrite((D3D12MA_RW_MUTEX*)Unsafe.AsPointer(ref m_Mutex), m_hAllocator->UseMutex());
        ...
        [return: NativeTypeName("HRESULT")]
        public int CreateResource([NativeTypeName("UINT64")] ulong size, [NativeTypeName("UINT64")] ulong alignment, D3D12MA_ALLOCATION_DESC* allocDesc, D3D12_RESOURCE_DESC* resourceDesc, D3D12_RESOURCE_STATES InitialResourceState, D3D12_CLEAR_VALUE* pOptimizedClearValue, D3D12MA_Allocation** ppAllocation, [NativeTypeName("REFIID")] Guid* riidResource, void** ppvResource)
        {
        ...
        [return: NativeTypeName("HRESULT")]
        public int CreateResource2([NativeTypeName("UINT64")] ulong size, [NativeTypeName("UINT64")] ulong alignment, D3D12MA_ALLOCATION_DESC* allocDesc, D3D12_RESOURCE_DESC1* resourceDesc, D3D12_RESOURCE_STATES InitialResourceState, D3D12_CLEAR_VALUE* pOptimizedClearValue, ID3D12ProtectedResourceSession *pProtectedSession, D3D12MA_Allocation** ppAllocation, [NativeTypeName("REFIID")] Guid* riidResource, void** ppvResource)
        {
        ...
        public int SetMinBytes([NativeTypeName("UINT64")] ulong minBytes)
        {
            ...
            if (minBytes < m_MinBytes)
            {
                ...
                {
                    D3D12MA_NormalBlock* block = *m_Blocks[blockIndex];
        ...
        public void AddStats(D3D12MA_StatInfo* outStats)
        {
            using var @lock = new D3D12MA_MutexLockRead((D3D12MA_RW_MUTEX*)Unsafe.AsPointer(ref m_Mutex), m_hAllocator->UseMutex());

            for (nuint i = 0; i < m_Blocks.size(); ++i)
            {
                D3D12MA_NormalBlock* pBlock = *m_Blocks[i];
        ...
        public void AddStats(D3D12MA_Stats* outStats)
        {
        ...
        public void WriteBlockInfoToJson(D3D12MA_JsonWriter* json)
        {
            using var @lock = new D3D12MA_MutexLockRead((D3D12MA_RW_MUTEX*)Unsafe.AsPointer(ref m_Mutex), m_hAllocator->UseMutex());
        ...
        void Remove(D3D12MA_NormalBlock* pBlock)
        {
        ...
        [return: NativeTypeName("HRESULT")]
        private int AllocatePage([NativeTypeName("UINT64")] ulong size, [NativeTypeName("UINT64")] ulong alignment, D3D12MA_ALLOCATION_DESC* allocDesc, D3D12MA_Allocation** pAllocation)
        {
            ...
            bool canCreateNewBlock =
                ((allocDesc->Flags & D3D12MA_ALLOCATION_FLAG_NEVER_ALLOCATE) == 0) &&
            ...
            // 1. Search existing allocations
            {
                ...
                for (nuint blockIndex = 0; blockIndex < m_Blocks.size(); ++blockIndex)
                {
                    D3D12MA_NormalBlock* pCurrBlock = *m_Blocks[blockIndex];
            ...
            if (canCreateNewBlock)
            {
                ...
                if (SUCCEEDED(hr))
                {
                    D3D12MA_NormalBlock* pBlock = *m_Blocks[newBlockIndex];
        ...
        private int CreateBlock([NativeTypeName("UINT64")] ulong blockSize, [NativeTypeName("size_t")] nuint* pNewBlockIndex)
        {
            ...
            *pBlock = new D3D12MA_NormalBlock(
                m_hAllocator,
                (D3D12MA_BlockVector*)Unsafe.AsPointer(ref this),
                m_HeapType,
                m_HeapFlags,
                blockSize,
                m_NextBlockId++);
            ...
            m_Blocks.push_back((Pointer<D3D12MA_NormalBlock>*)&pBlock);
