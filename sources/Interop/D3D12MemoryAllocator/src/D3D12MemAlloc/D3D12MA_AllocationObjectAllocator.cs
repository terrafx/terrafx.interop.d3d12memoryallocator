...
internal unsafe partial struct D3D12MA_AllocationObjectAllocator : IDisposable
{
    ...
    public D3D12MA_AllocationObjectAllocator([NativeTypeName("const D3D12MA::ALLOCATION_CALLBACKS &")] in D3D12MA_ALLOCATION_CALLBACKS allocationCallbacks) : this()
    {
    ...
    public D3D12MA_Allocation* Allocate(D3D12MA_AllocatorPimpl* allocator, [NativeTypeName("UINT64")] ulong size, [NativeTypeName("UINT64")] ulong alignment, BOOL wasZeroInitialized)
    {
        using D3D12MA_MutexLock mutexLock = new D3D12MA_MutexLock(ref m_Mutex);
        return m_Allocator.Alloc(allocator, size, alignment, wasZeroInitialized);
    }
