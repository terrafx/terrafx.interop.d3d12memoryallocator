...
using AllocationVectorType = TerraFX.Interop.D3D12MA_Vector<TerraFX.Interop.Pointer<TerraFX.Interop.D3D12MA_Allocation>>;
using PoolVectorType = TerraFX.Interop.D3D12MA_Vector<TerraFX.Interop.Pointer<TerraFX.Interop.D3D12MA_Pool>>;
...
#pragma warning restore CS0649
        ...
        D3D12MA_ALLOCATION_CALLBACKS m_AllocationCallbacks;
        ...
        D3D12MA_AllocationObjectAllocator m_AllocationObjectAllocator;

        [NativeTypeName("AllocationVectorType* m_pCommittedAllocations[HEAP_TYPE_COUNT]")]
        __m_Buffer_HEAP_TYPE_COUNT_e__FixedBuffer<Pointer<AllocationVectorType>> m_pCommittedAllocations;
        ...
        __m_Buffer_HEAP_TYPE_COUNT_e__FixedBuffer<Pointer<PoolVectorType>> m_pPools;
        ...
        __m_Buffer_DEFAULT_POOL_MAX_COUNT_e__FixedBuffer<Pointer<D3D12MA_BlockVector>> m_BlockVectors;
        ...
        public void Ctor(D3D12MA_ALLOCATION_CALLBACKS* allocationCallbacks, D3D12MA_ALLOCATOR_DESC* desc)
        {
            Unsafe.AsRef(m_UseMutex) = ((int)desc->Flags & (int)D3D12MA_ALLOCATOR_FLAG_SINGLETHREADED) == 0;
            Unsafe.AsRef(m_AlwaysCommitted) = ((int)desc->Flags & (int)D3D12MA_ALLOCATOR_FLAG_ALWAYS_COMMITTED) != 0;
            ...
            m_AllocationObjectAllocator = new D3D12MA_AllocationObjectAllocator((D3D12MA_ALLOCATION_CALLBACKS*)Unsafe.AsPointer(ref m_AllocationCallbacks));
        ...
        public int Init(D3D12MA_ALLOCATOR_DESC* desc)
        {
            ...
            for (uint i = 0; i < defaultPoolCount; ++i)
            {
                ...
                D3D12MA_BlockVector* p = m_BlockVectors[(int)i] = D3D12MA_NEW<D3D12MA_BlockVector>(GetAllocs());
                *p = new D3D12MA_BlockVector(
                    pThis, // hAllocator
                    heapType, // heapType
                    heapFlags, // heapFlags
        ...
        public readonly D3D12MA_ALLOCATION_CALLBACKS* GetAllocs() => (D3D12MA_ALLOCATION_CALLBACKS*)Unsafe.AsPointer(ref Unsafe.AsRef(m_AllocationCallbacks));
            ...
            *ppAllocation = null;
            ...
            else
            {
                ...
                if (preferCommittedMemory &&
                    (finalAllocDesc.Flags & D3D12MA_ALLOCATION_FLAG_NEVER_ALLOCATE) == 0)
                {
        ...
        public int CreateResource1(D3D12MA_ALLOCATION_DESC* pAllocDesc, D3D12_RESOURCE_DESC* pResourceDesc, D3D12_RESOURCE_STATES InitialResourceState, D3D12_CLEAR_VALUE* pOptimizedClearValue, ID3D12ProtectedResourceSession *pProtectedSession, D3D12MA_Allocation** ppAllocation, [NativeTypeName("REFIID")] Guid* riidResource, void** ppvResource)
        {
            ...
            *ppAllocation = null;
            ...
            if (pAllocDesc->CustomPool != null ||
                (pAllocDesc->Flags & D3D12MA_ALLOCATION_FLAG_NEVER_ALLOCATE) != 0)
            {
                return E_INVALIDARG;
            }
        ...
        [return: NativeTypeName("HRESULT")]
        public int CreateAliasingResource(D3D12MA_Allocation* pAllocation, [NativeTypeName("UINT64")] ulong AllocationLocalOffset, D3D12_RESOURCE_DESC* pResourceDesc, D3D12_RESOURCE_STATES InitialResourceState, D3D12_CLEAR_VALUE* pOptimizedClearValue, [NativeTypeName("REFIID")] Guid* riidResource, void** ppvResource)
        {
            *ppvResource = null;
        ...
        public int SetDefaultHeapMinBytes(D3D12_HEAP_TYPE heapType, D3D12_HEAP_FLAGS heapFlags, [NativeTypeName("UINT64")] ulong minBytes)
        {
            ...
            if (SupportsResourceHeapTier2())
            {
                ...
                {
                    using var @lock = new D3D12MA_MutexLockWrite((D3D12MA_RW_MUTEX*)Unsafe.AsPointer(ref m_DefaultPoolMinBytesMutex), m_UseMutex);
        ...
        {
            D3D12MA_Allocation* pAllocation = (D3D12MA_Allocation*)Unsafe.AsPointer(ref allocation);
            D3D12MA_ASSERT((D3D12MA_DEBUG_LEVEL > 0) && (pAllocation != null && pAllocation->m_PackedData.GetType() == D3D12MA_Allocation.Type.TYPE_COMMITTED));
        ...
        {
            D3D12MA_Allocation* pAllocation = (D3D12MA_Allocation*)Unsafe.AsPointer(ref allocation);
            D3D12MA_ASSERT((D3D12MA_DEBUG_LEVEL > 0) && (pAllocation != null && pAllocation->m_PackedData.GetType() == D3D12MA_Allocation.Type.TYPE_PLACED));

            D3D12MA_NormalBlock* block = pAllocation->m_Placed.block;
        ...
        {
            D3D12MA_Allocation* pAllocation = (D3D12MA_Allocation*)Unsafe.AsPointer(ref allocation);
            D3D12MA_ASSERT((D3D12MA_DEBUG_LEVEL > 0) && (pAllocation != null && pAllocation->m_PackedData.GetType() == D3D12MA_Allocation.Type.TYPE_HEAP));
        ...
        public void CalculateStats(D3D12MA_Stats* outStats)
        {
            ...
            for (nuint heapTypeIndex = 0; heapTypeIndex < HEAP_TYPE_COUNT; ++heapTypeIndex)
            {
                using var @lock = new D3D12MA_MutexLockRead((D3D12MA_RW_MUTEX*)Unsafe.AsPointer(ref m_PoolsMutex[(int)heapTypeIndex]), m_UseMutex);
                ...
                {
                    D3D12MA_Pool* pool = *(*poolVector)[poolIndex];
            ...
            for (nuint heapTypeIndex = 0; heapTypeIndex < HEAP_TYPE_COUNT; ++heapTypeIndex)
            {
                ...
                using var @lock = new D3D12MA_MutexLockRead((D3D12MA_RW_MUTEX*)Unsafe.AsPointer(ref m_CommittedAllocationsMutex[(int)heapTypeIndex]), m_UseMutex);
        ...
        public void GetBudget(D3D12MA_Budget* outGpuBudget, D3D12MA_Budget* outCpuBudget)
        {
            ...
            if (D3D12MA_DXGI_1_4 > 0)
            {
                if (m_Adapter3 != null)
                {
                    ...
                    {
                        using var @lock = new D3D12MA_MutexLockRead((D3D12MA_RW_MUTEX*)Unsafe.AsPointer(ref m_Budget.m_BudgetMutex), m_UseMutex);
                        if (outGpuBudget != null)
                        {
        ...
        static void AddStatInfoToJson(D3D12MA_JsonWriter* json, D3D12MA_StatInfo* statInfo)
        {
            json->BeginObject();
        ...
        public void BuildStatsString([NativeTypeName("WCHAR**")] ushort** ppStatsString, [NativeTypeName("BOOL")] int DetailedMap)
        {
            ...
            using (var json = new D3D12MA_JsonWriter(GetAllocs(), &sb))
            {
                ...
                if (DetailedMap > 0)
                {
                    ...
                    {
                        json.WriteString(HeapTypeNames[heapType]);
                        using var @lock = new D3D12MA_MutexLockRead((D3D12MA_RW_MUTEX*)Unsafe.AsPointer(ref m_CommittedAllocationsMutex[(int)heapType]), m_UseMutex);
                        ...
                        {
                            D3D12MA_Allocation* alloc = *(*allocationVector)[i];
        ...
        [return: NativeTypeName("UINT")]
        private uint CalcDefaultPoolIndex([NativeTypeName("const ALLOCATION_DESC&")] D3D12MA_ALLOCATION_DESC* allocDesc, [NativeTypeName("const D3D12_RESOURCE_DESC_T&")] D3D12_RESOURCE_DESC* resourceDesc)
        {
        ...
        [return: NativeTypeName("UINT")]
        private uint CalcDefaultPoolIndex([NativeTypeName("const ALLOCATION_DESC&")] D3D12MA_ALLOCATION_DESC* allocDesc, [NativeTypeName("const D3D12_RESOURCE_DESC_T&")] D3D12_RESOURCE_DESC1* resourceDesc)
        {
        ...
        [return: NativeTypeName("UINT")]
        uint CalcDefaultPoolIndex(D3D12MA_ALLOCATION_DESC* allocDesc)
        {
        ...
        void RegisterCommittedAllocation(D3D12MA_Allocation* alloc, D3D12_HEAP_TYPE heapType)
        {
            uint heapTypeIndex = HeapTypeToIndex(heapType);

            using var @lock = new D3D12MA_MutexLockWrite((D3D12MA_RW_MUTEX*)Unsafe.AsPointer(ref m_CommittedAllocationsMutex[(int)heapTypeIndex]), m_UseMutex);
            ...
            committedAllocations->InsertSorted((Pointer<D3D12MA_Allocation>*)&alloc, new PointerLess());
        }

        /// <summary>Unregisters Allocation object from m_pCommittedAllocations.</summary>
        void UnregisterCommittedAllocation(D3D12MA_Allocation* alloc, D3D12_HEAP_TYPE heapType)
        {
            uint heapTypeIndex = HeapTypeToIndex(heapType);
            ...
            AllocationVectorType* committedAllocations = m_pCommittedAllocations[(int)heapTypeIndex];
            D3D12MA_ASSERT((D3D12MA_DEBUG_LEVEL > 0) && (committedAllocations != null));
            bool success = committedAllocations->RemoveSorted((Pointer<D3D12MA_Allocation>*)&alloc, new PointerLess());
        ...
        internal void RegisterPool(D3D12MA_Pool* pool, D3D12_HEAP_TYPE heapType)
        {
            ...
            using var @lock = new D3D12MA_MutexLockWrite((D3D12MA_RW_MUTEX*)Unsafe.AsPointer(ref m_PoolsMutex[(int)heapTypeIndex]), m_UseMutex);
            PoolVectorType* pools = m_pPools[(int)heapTypeIndex];
            D3D12MA_ASSERT((D3D12MA_DEBUG_LEVEL > 0) && (pools != null));
            pools->InsertSorted((Pointer<D3D12MA_Pool>*)&pool, new PointerLess());
        }
        ...
        internal void UnregisterPool([NativeTypeName("Pool*")] ref D3D12MA_Pool pool, D3D12_HEAP_TYPE heapType)
        {
            fixed (D3D12MA_Pool* pPool = &pool)
            {
                ...
                PoolVectorType* pools = m_pPools[(int)heapTypeIndex];
                D3D12MA_ASSERT((D3D12MA_DEBUG_LEVEL > 0) && (pools != null));
                bool success = pools->RemoveSorted((Pointer<D3D12MA_Pool>*)&pPool, new PointerLess());
                D3D12MA_ASSERT((D3D12MA_DEBUG_LEVEL > 0) && (success));
            }
        }
        ...
        private int UpdateD3D12Budget()
        {
            if (D3D12MA_DXGI_1_4 > 0)
            {
                ...
                {
                    using var lockWrite = new D3D12MA_MutexLockWrite((D3D12MA_RW_MUTEX*)Unsafe.AsPointer(ref m_Budget.m_BudgetMutex), m_UseMutex);
